...
import (
	...
	coredeploymentclientset "github.com/soniabha-intc/edgedeploy/pkg/crd/coredeployment/v1alpha1/apis/clientset/versioned"
...
var (
	onceDefaultCoredeployV1Alpha1Client sync.Once
...
type Client struct {
	clientset *coredeploymentclientset.Clientset
	namespace string
	plural    string
	ctx       context.Context
}
...
// CreateCoreDeploymentClientset returns the clientset for CRD AppDeployment v1alpha1 in singleton way.
func CreateCoreDeploymentClientset(config *rest.Config) (*coredeploymentclientset.Clientset, error) {
	/*restConfig, err := clientcmd.BuildConfigFromFlags("", kubeconfigPath)
	if err != nil {
		return nil, err
	}*/
	clientset, err := coredeploymentclientset.NewForConfig(config)
	if err != nil {
		return nil, err
	}
...
// NewClient accepts kubeconfig path and namespace. Return the API client interface for CRD CoreDeployment v1alpha1.
func NewClient(ctx context.Context, config *rest.Config, namespace string) (*Client, error) {
	clientset, err := CreateCoreDeploymentClientset(config)
	if err != nil {
...
// GetDefaultClient returns an API client interface for CRD CoreDeployment v1alpha1. It assumes the kubeconfig
// is available at default path and the target CRD namespace is the default namespace.
func GetDefaultClient() *Client {
	onceDefaultCoredeployV1Alpha1Client.Do(func() {
		...
		clientset, err := CreateCoreDeploymentClientset(kconfig)
		if err != nil {
			panic("Fail to init default CRD API client for CoreDeployment v1alpha1: " + err.Error())
		}
		defaultClient = &Client{
			clientset: clientset,
